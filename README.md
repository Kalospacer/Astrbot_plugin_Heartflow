...
1. 新消息照常记进上下文缓冲，然后等待 `debounce_seconds`。
2. 等待期间若又来了新消息，判断权交给最新那条，先前那条直接放弃——它已经在缓冲里，最新那条看得到。
3. 窗口结束时仍是最新的那条才真正发起判断，`current_message` 是它，同窗口的其余消息落在 `chat_log` 里。

所以一个窗口只产生一次判断调用。群里连刷 5 条，判断从 5 次降到 1 次，回复也不会在别人话说到一半时插进去。

注意：

- 触发回复时，回复的是窗口内最后那条消息，机器人发言会比最后一条晚 `debounce_seconds` 秒，这是防抖的固有代价。建议从 `3`~`8` 秒起步。
- 直接 @ 机器人的消息由 AstrBot 核心唤醒接管，根本不进心流判断，因此**不受防抖影响**，@ 了仍是秒回。
- 一个窗口能被判断看到的消息条数受 `judge_context_count` 限制；窗口开得大就把它一并调大，否则刷屏的前几条会被截掉。
